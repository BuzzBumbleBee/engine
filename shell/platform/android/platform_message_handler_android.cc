...
bool PlatformMessageHandlerAndroid::SendToPlatformPortCallback(
  ...
  if (it == active_callbacks_.end()) {
    FML_LOG(ERROR) << "No isolate callbacks set for " << name << " channel.";
    return false;
  }

  pending_callback_responses_[response_id] = response;
...
void PlatformMessageHandlerAndroid::AddPlatformPortCallback(
    int64_t port,
    std::string channel) {
...
void PlatformMessageHandlerAndroid::RemovePlatformPortCallback(
    std::string channel) {
  std::lock_guard lock(pending_callback_responses_mutex_);
  auto it = active_callbacks_.find(channel);
  if (it == active_callbacks_.end()) {
    FML_LOG(ERROR) << "No isolate callbacks set for " << channel << " channel.";
    return;
  }
  FML_LOG(ERROR) << "PlatformMessageHandlerAndroid::AddPlatformPortCallback "
                 << channel << " removed.";
